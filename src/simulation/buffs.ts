...
export function applyBuff(entityId: number, buff: Buff): void {
  ...
  const existingIdx = buffs.findIndex(b => 
    b.source === buff.source && b.sourceId === buff.sourceId
  );
 ...
 */
export function removeBuffsBySource(entityId: number, source: string, sourceId: number): boolean {
  const buffs = activeBuffs.get(entityId);
  if (!buffs) return false;
  
  const beforeLength = buffs.length;
  const filtered = buffs.filter(b => !(b.source === source && b.sourceId === sourceId));
...
export function clearExpiredBuffs(entityId: number): number {
  ...
  const currentDay = S.gameYear * C.DAYS_PER_MONTH + S.gameMonth * C.DAYS_PER_MONTH + S.gameDay;
  const beforeLength = buffs.length;
