...
export function runGatheringSystem(
      ...
      if (dx * dx + dy * dy < 4.0) {
        ...
        if (S.actionTimer[i] <= 0) {
          if (bldId !== -1 && S.bldType[bldId] === C.BuildingType.Field) {
            ...
            if ((traits & C.TRAIT_BUSH) !== 0) {
              S.entityInventory[i] = 10;
              S.charTool[i] = 2;
            } else if ((traits & C.TRAIT_TREE) !== 0) {
              S.entityInventory[i] = 50;
              S.charTool[i] = 0;
            } else if ((traits & C.TRAIT_GOLD) !== 0) {
              S.entityInventory[i] = 100;
              S.charTool[i] = 1;
            }
          }
        ...
        if (S.state[i] === C.EntityState.ReturningToDepot) {
          ...
          if (S.bldType[bldId] === C.BuildingType.Warehouse) {
            ...
            const amountToDeposit = Math.min(
              S.entityInventory[i],
              Math.max(0, limit - currentAmount),
            );

            if (amountToDeposit > 0) {
              if (invSlot === 0)
                Atomics.add(S.bldDataA, bldId, amountToDeposit);
              else if (invSlot === 1)
                Atomics.add(S.bldDataB, bldId, amountToDeposit);
              else if (invSlot === 2)
                Atomics.add(S.bldDataC, bldId, amountToDeposit);
  ...
  if (S.tickCount % 60 === 0) {
    const bldStart = Math.floor(
      (startIndex / C.MAX_ENTITIES) * C.MAX_BUILDINGS,
    );
