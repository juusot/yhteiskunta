 ...
 */
export function runCombatSystem(state: SharedArrayBuffer, startIndex: number, endIndex: number): void {
  // 1. Projectile Processing
  const projStart = Math.floor((startIndex / C.MAX_ENTITIES) * C.MAX_PROJECTILES);
        ...
        if (S.health[victimId] > 0 && S.state[victimId] !== C.EntityState.Dead) {
          ...
          if (isEnemy) {
            ...
            if (dx * dx + dy * dy < 16.0) { 
              ...
              if (ownerGroup !== -1) {
                  S.targetEntityId[victimId] = ownerGroup >= 0 ? S.groupWarehouseX[ownerGroup] : -1; 
              }
              S.projType[i] = 0; 
              S.projLifeTime[i] = 0; 
              break; 
          ...
          if (tx >= 0 && tx < C.GRID_COLS && ty >= 0 && ty < C.GRID_ROWS) {
            ...
            while (entityId !== -1) {
              if (S.health[entityId] > 0 && S.state[entityId] !== C.EntityState.Dead) {
                const ex = S.positionX[entityId], ey = S.positionY[entityId];
                const ddx = ex - bx, ddy = ey - by;
                if (ddx * ddx + ddy * ddy < rangeSq) {
                  S.groupAffiliations[entityId * C.MAX_GROUP_CHANNELS + 0] = ownerGroup;
                }
              }
