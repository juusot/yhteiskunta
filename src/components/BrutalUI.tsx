...
export const BrutalButton: React.FC<{
  ...
  const baseClasses = "px-4 py-1 font-mono text-[12px] font-bold uppercase transition-colors duration-100 border-2 border-on-surface flex items-center justify-center gap-2";
  
  const variants = {
    primary: "bg-primary-container text-on-primary-container shadow-brutal-sm hover:bg-on-surface hover:text-primary-container",
    secondary: "bg-secondary-container text-on-secondary-container shadow-brutal-sm hover:bg-on-surface hover:text-secondary-container",
    error: "bg-error text-on-error hover:bg-on-error-container",
    surface: "bg-surface text-on-surface shadow-brutal-sm hover:bg-on-surface hover:text-surface",
    ghost: "bg-transparent text-on-surface border-transparent hover:bg-on-surface/10 shadow-none",
  };

  // Active state uses Yellow background (primary-container) for maximum visibility
  const activeClasses = active ? "bg-primary-container text-on-primary-container shadow-none translate-x-[2px] translate-y-[2px] border-4" : "";
    ...
    <div className="w-full overflow-x-auto border-2 border-on-surface bg-surface-container-lowest">
      <table className="w-full text-left font-mono text-[13px]">
        <thead className="bg-surface-container-high border-b border-on-surface">
          <tr>
            {headers.map((h, i) => (
              <th key={i} className="p-2 border-r border-surface-variant font-bold uppercase">
                {h}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="border-b border-surface-variant last:border-0 hover:bg-surface-container">
              {row.map((cell, j) => (
                <td key={j} className="p-2 border-r border-surface-variant last:border-0">
                  {cell}
                </td>
              ))}
            </tr>
