...
import {
  MAX_ENTITIES,
  MAX_GROUPS,
  WORLD_MAP_COLS as MAP_COLS,
  WORLD_MAP_ROWS as MAP_ROWS,
  MAX_BUILDINGS,
  MAX_VEHICLES,
  MAX_PROJECTILES,
  MAX_ITEM_INSTANCES,
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (typeof SharedArrayBuffer === "undefined") {
    const msg =
      "SharedArrayBuffer is not supported. This game requires a cross-origin isolated environment.";
    console.error(msg);
    alert(msg);
    return;
  }
  ...
  if (!gl) {
    ...
    alert(
      "WebGL 2 not supported. On Linux, try --ignore-gpu-blocklist in your browser, or check your GPU drivers.",
    );
    return;
  }
  ...
  try {
    ...
    alert(
      "Failed to spawn workers. Your browser might not support ES module workers.",
    );
    return;
  }

  const uiWorker = new Worker(new URL("./uiWorker.ts", import.meta.url), {
    type: "module",
  });
  ...
  const btnTerritoryToggle = document.getElementById(
    "btn-territory-toggle",
  ) as HTMLButtonElement;
  const gameTimeDisplay = document.getElementById(
    "gameTimeDisplay",
  ) as HTMLDivElement;
  const btnUiToggle = document.getElementById(
    "btn-ui-toggle",
  ) as HTMLButtonElement;
  const btnUiClose = document.getElementById(
    "btn-ui-close",
  ) as HTMLButtonElement;
  ...
  // UI Popup toggle
  btnUiToggle.onclick = () => {
    uiPopup.classList.add("visible");
  };
  btnUiClose.onclick = () => {
    uiPopup.classList.remove("visible");
  };
  ...
  let positionX: Float32Array,
    ...
    traitBitmask: Uint32Array,
    groupAffiliations: Int32Array;
  let health: Int32Array,
    money: Int32Array,
    state: Uint8Array,
    entityInventory: Int16Array;
  let worldMap: Uint8Array,
    territoryOwnerMap: Int32Array,
    influenceMap: Int16Array,
    workerSync: Int32Array;
  let ruleRegistry: Int32Array,
    logicBytecode: Int32Array,
    groupPopulation: Int32Array,
    groupTotalWealth: Int32Array,
    groupBuildingCount: Int32Array;
  let groupWood: Int32Array,
    groupGold: Int32Array,
    groupFood: Int32Array,
    groupMisc: Int32Array;
  let bldPositionX: Float32Array,
    bldPositionY: Float32Array,
    bldType: Uint8Array,
    bldHealth: Int32Array,
    bldOwnerGroup: Int32Array,
    bldTier: Uint8Array;
  let vehPositionX: Float32Array,
    vehPositionY: Float32Array,
    vehType: Uint8Array,
    vehOwnerGroup: Int32Array;
  let playerTargetX: Float32Array,
    playerTargetY: Float32Array,
    scenarioState: Int32Array;
  let projPositionX: Float32Array,
    projPositionY: Float32Array,
    projType: Uint8Array,
    projLifeTime: Int16Array;
  let itemInstanceX: Float32Array,
    itemInstanceY: Float32Array,
  ...
  const vehVao = gl.createVertexArray();
  gl.bindVertexArray(vehVao);
  gl.enableVertexAttribArray(0);
  gl.vertexAttribPointer(0, 2, gl.FLOAT, false, 0, 0);

  const instPosXVbo = gl.createBuffer(),
    instPosYVbo = gl.createBuffer(),
    instTypeVbo = gl.createBuffer(),
    instGroupVbo = gl.createBuffer(),
    instHealthVbo = gl.createBuffer();
  const bldPosXVbo = gl.createBuffer(),
    bldPosYVbo = gl.createBuffer(),
    bldTypeVbo = gl.createBuffer(),
    bldGroupVbo = gl.createBuffer(),
    bldHealthVbo = gl.createBuffer();
  const vehPosXVbo = gl.createBuffer(),
    vehPosYVbo = gl.createBuffer(),
    vehTypeVbo = gl.createBuffer(),
    vehGroupVbo = gl.createBuffer();
  const projPosXVbo = gl.createBuffer(),
    projPosYVbo = gl.createBuffer(),
    projTypeVbo = gl.createBuffer();
  const itemPosXVbo = gl.createBuffer(),
    itemPosYVbo = gl.createBuffer(),
    itemOwnerTypeVbo = gl.createBuffer(),
    itemDefIdVbo = gl.createBuffer();
  const tileTypeVbo = gl.createBuffer(),
    infOwnerVbo = gl.createBuffer(),
    infStrengthVbo = gl.createBuffer(),
    infBorderVbo = gl.createBuffer();
  ...
  gl.enableVertexAttribArray(0);
  gl.vertexAttribPointer(0, 2, gl.FLOAT, false, 0, 0);
  gl.bindBuffer(gl.ARRAY_BUFFER, projPosXVbo);
  gl.enableVertexAttribArray(1);
  gl.vertexAttribPointer(1, 1, gl.FLOAT, false, 0, 0);
  gl.vertexAttribDivisor(1, 1);
  gl.bindBuffer(gl.ARRAY_BUFFER, projPosYVbo);
  gl.enableVertexAttribArray(2);
  gl.vertexAttribPointer(2, 1, gl.FLOAT, false, 0, 0);
  gl.vertexAttribDivisor(2, 1);
  gl.bindBuffer(gl.ARRAY_BUFFER, projTypeVbo);
  gl.enableVertexAttribArray(3);
  gl.vertexAttribPointer(3, 1, gl.UNSIGNED_BYTE, false, 0, 0);
  gl.vertexAttribDivisor(3, 1);

  const itemVao = gl.createVertexArray();
  gl.bindVertexArray(itemVao);
  gl.bindBuffer(gl.ARRAY_BUFFER, quadVbo);
  gl.enableVertexAttribArray(0);
  gl.vertexAttribPointer(0, 2, gl.FLOAT, false, 0, 0);
  gl.bindBuffer(gl.ARRAY_BUFFER, itemPosXVbo);
  gl.enableVertexAttribArray(1);
  gl.vertexAttribPointer(1, 1, gl.FLOAT, false, 0, 0);
  gl.vertexAttribDivisor(1, 1);
  gl.bindBuffer(gl.ARRAY_BUFFER, itemPosYVbo);
  gl.enableVertexAttribArray(2);
  gl.vertexAttribPointer(2, 1, gl.FLOAT, false, 0, 0);
  gl.vertexAttribDivisor(2, 1);
  gl.bindBuffer(gl.ARRAY_BUFFER, itemOwnerTypeVbo);
  gl.enableVertexAttribArray(3);
  gl.vertexAttribPointer(3, 1, gl.UNSIGNED_BYTE, false, 0, 0);
  gl.vertexAttribDivisor(3, 1);
  gl.bindBuffer(gl.ARRAY_BUFFER, itemDefIdVbo);
  gl.enableVertexAttribArray(4);
  gl.vertexAttribPointer(4, 1, gl.UNSIGNED_SHORT, false, 0, 0);
  gl.vertexAttribDivisor(4, 1);

  const tileTypeBuffer = new Float32Array(MAP_COLS * MAP_ROWS),
    infOwnerBuffer = new Float32Array(MAP_COLS * MAP_ROWS),
    infStrengthBuffer = new Float32Array(MAP_COLS * MAP_ROWS),
    infBorderBuffer = new Float32Array(MAP_COLS * MAP_ROWS);
  ...
  const locs = {
    tile: {
      res: gl.getUniformLocation(tileProg, "u_resolution"),
      cam: gl.getUniformLocation(tileProg, "u_camera"),
    },
    inf: {
      res: gl.getUniformLocation(infProg, "u_resolution"),
      cam: gl.getUniformLocation(infProg, "u_camera"),
      zoom: gl.getUniformLocation(infProg, "u_zoom"),
    },
    ent: {
      res: gl.getUniformLocation(entProg, "u_resolution"),
      cam: gl.getUniformLocation(entProg, "u_camera"),
    },
    bld: {
      res: gl.getUniformLocation(bldProg, "u_resolution"),
      cam: gl.getUniformLocation(bldProg, "u_camera"),
    },
    veh: {
      res: gl.getUniformLocation(vehProg, "u_resolution"),
      cam: gl.getUniformLocation(vehProg, "u_camera"),
    },
    proj: {
      res: gl.getUniformLocation(projProg, "u_resolution"),
      cam: gl.getUniformLocation(projProg, "u_camera"),
    },
    item: {
      res: gl.getUniformLocation(itemProg, "u_resolution"),
      cam: gl.getUniformLocation(itemProg, "u_camera"),
    },
  };
  ...
  window.addEventListener(
    ...
    (e) => {
      if (
        ...
        document.body.dataset.uiOpen === "true"
      )
        return;
      const rect = canvas.getBoundingClientRect();
  ...
  canvas.addEventListener("click", (e) => {
    ...
    if ((window as any).brushState?.active) {
      workers.forEach((w) =>
        w.postMessage({
          ...
          payload: {
            ...
            y: worldY,
            radius: 50,
            groupId: (window as any).brushState.groupId,
            traitBitmask: (window as any).brushState.trait,
          },
        }),
      );
    } else {
      ...
      workers[0].postMessage({
        type: "FIND_ENTITY",
        payload: { x: worldX, y: worldY, radius: 20 },
      });
    }
  });
  ...
  btnToggleLoop.onclick = () => {
    isLooping = !isLooping;
    btnToggleLoop.textContent = isLooping ? "⏸" : "▶";
    if (isLooping) startTick();
  };
  btnTerritoryToggle.onclick = () => {
  ...
  function render() {
    ...
    gl.uniform2fv(locs.tile.res, uRes);
    gl.uniform4fv(locs.tile.cam, uCam);
    for (let i = 0; i < MAP_COLS * MAP_ROWS; i++)
      tileTypeBuffer[i] = worldMap[i];
    gl.bindBuffer(gl.ARRAY_BUFFER, tileTypeVbo);
    gl.bufferData(gl.ARRAY_BUFFER, tileTypeBuffer, gl.DYNAMIC_DRAW);
    gl.bindVertexArray(tileVao);
    gl.enableVertexAttribArray(1);
    gl.vertexAttribPointer(1, 1, gl.FLOAT, false, 0, 0);
    gl.vertexAttribDivisor(1, 1);
    ...
    if (showPoliticalMap) {
      ...
      gl.uniform2fv(locs.inf.res, uRes);
      gl.uniform4fv(locs.inf.cam, uCam);
      ...
      for (let y = 0; y < MAP_ROWS; y++) {
        for (let x = 0; x < MAP_COLS; x++) {
          ...
          if (showBorders && owner !== -1) {
            ...
            else if (x < MAP_COLS - 1 && territoryOwnerMap[i + 1] !== owner)
              isBorder = 1;
            else if (y > 0 && territoryOwnerMap[i - MAP_COLS] !== owner)
              isBorder = 1;
            else if (
              y < MAP_ROWS - 1 &&
              territoryOwnerMap[i + MAP_COLS] !== owner
            )
              isBorder = 1;
          }
      ...
      gl.bindBuffer(gl.ARRAY_BUFFER, infOwnerVbo);
      gl.bufferData(gl.ARRAY_BUFFER, infOwnerBuffer, gl.DYNAMIC_DRAW);
      gl.bindVertexArray(infVao);
      gl.enableVertexAttribArray(1);
      gl.vertexAttribPointer(1, 1, gl.FLOAT, false, 0, 0);
      gl.vertexAttribDivisor(1, 1);
      gl.bindBuffer(gl.ARRAY_BUFFER, infStrengthVbo);
      gl.bufferData(gl.ARRAY_BUFFER, infStrengthBuffer, gl.DYNAMIC_DRAW);
      gl.enableVertexAttribArray(2);
      gl.vertexAttribPointer(2, 1, gl.FLOAT, false, 0, 0);
      gl.vertexAttribDivisor(2, 1);

      // Add border buffer
      gl.bindBuffer(gl.ARRAY_BUFFER, infBorderVbo);
      gl.bufferData(gl.ARRAY_BUFFER, infBorderBuffer, gl.DYNAMIC_DRAW);
      gl.enableVertexAttribArray(3);
    ...
    if (bldPositionX) {
      ...
      gl.bindVertexArray(bldVao);
      gl.bindBuffer(gl.ARRAY_BUFFER, bldPosXVbo);
      gl.bufferData(gl.ARRAY_BUFFER, bldPositionX, gl.DYNAMIC_DRAW);
      gl.enableVertexAttribArray(1);
      gl.vertexAttribPointer(1, 1, gl.FLOAT, false, 0, 0);
      gl.vertexAttribDivisor(1, 1);
      gl.bindBuffer(gl.ARRAY_BUFFER, bldPosYVbo);
      gl.bufferData(gl.ARRAY_BUFFER, bldPositionY, gl.DYNAMIC_DRAW);
      gl.enableVertexAttribArray(2);
      gl.vertexAttribPointer(2, 1, gl.FLOAT, false, 0, 0);
      gl.vertexAttribDivisor(2, 1);
      gl.bindBuffer(gl.ARRAY_BUFFER, bldTypeVbo);
      gl.bufferData(gl.ARRAY_BUFFER, bldType, gl.DYNAMIC_DRAW);
      gl.enableVertexAttribArray(3);
      gl.vertexAttribPointer(3, 1, gl.UNSIGNED_BYTE, false, 0, 0);
      gl.vertexAttribDivisor(3, 1);
      gl.bindBuffer(gl.ARRAY_BUFFER, bldGroupVbo);
      gl.bufferData(gl.ARRAY_BUFFER, bldOwnerGroup, gl.DYNAMIC_DRAW);
      gl.enableVertexAttribArray(4);
      gl.vertexAttribPointer(4, 1, gl.INT, false, 0, 0);
      gl.vertexAttribDivisor(4, 1);
      gl.bindBuffer(gl.ARRAY_BUFFER, bldHealthVbo);
      gl.bufferData(gl.ARRAY_BUFFER, bldHealth, gl.DYNAMIC_DRAW);
    ...
    gl.uniform2fv(locs.ent.res, uRes);
    gl.uniform4fv(locs.ent.cam, uCam);
    gl.bindVertexArray(entVao);
    gl.bindBuffer(gl.ARRAY_BUFFER, instPosXVbo);
    gl.bufferData(gl.ARRAY_BUFFER, positionX, gl.DYNAMIC_DRAW);
    gl.enableVertexAttribArray(1);
    gl.vertexAttribPointer(1, 1, gl.FLOAT, false, 0, 0);
    gl.vertexAttribDivisor(1, 1);
    gl.bindBuffer(gl.ARRAY_BUFFER, instPosYVbo);
    gl.bufferData(gl.ARRAY_BUFFER, positionY, gl.DYNAMIC_DRAW);
    gl.enableVertexAttribArray(2);
    gl.vertexAttribPointer(2, 1, gl.FLOAT, false, 0, 0);
    gl.vertexAttribDivisor(2, 1);
    gl.bindBuffer(gl.ARRAY_BUFFER, instTypeVbo);
    gl.bufferData(gl.ARRAY_BUFFER, traitBitmask, gl.DYNAMIC_DRAW);
    gl.enableVertexAttribArray(3);
    gl.vertexAttribPointer(3, 1, gl.UNSIGNED_INT, false, 0, 0);
    gl.vertexAttribDivisor(3, 1);
    ...
    if (vehPositionX) {
      ...
      gl.uniform2fv(locs.veh.res, uRes);
      gl.uniform4fv(locs.veh.cam, uCam);
      gl.bindVertexArray(vehVao);
      gl.bindBuffer(gl.ARRAY_BUFFER, vehPosXVbo);
      gl.bufferData(gl.ARRAY_BUFFER, vehPositionX, gl.DYNAMIC_DRAW);
      gl.enableVertexAttribArray(1);
      gl.vertexAttribPointer(1, 1, gl.FLOAT, false, 0, 0);
      gl.vertexAttribDivisor(1, 1);
      gl.bindBuffer(gl.ARRAY_BUFFER, vehPosYVbo);
      gl.bufferData(gl.ARRAY_BUFFER, vehPositionY, gl.DYNAMIC_DRAW);
      gl.enableVertexAttribArray(2);
      gl.vertexAttribPointer(2, 1, gl.FLOAT, false, 0, 0);
      gl.vertexAttribDivisor(2, 1);
      gl.bindBuffer(gl.ARRAY_BUFFER, vehTypeVbo);
      gl.bufferData(gl.ARRAY_BUFFER, vehType, gl.DYNAMIC_DRAW);
      gl.enableVertexAttribArray(3);
      gl.vertexAttribPointer(3, 1, gl.UNSIGNED_BYTE, false, 0, 0);
      gl.vertexAttribDivisor(3, 1);
      gl.bindBuffer(gl.ARRAY_BUFFER, vehGroupVbo);
      gl.bufferData(gl.ARRAY_BUFFER, vehOwnerGroup, gl.DYNAMIC_DRAW);
      gl.enableVertexAttribArray(4);
      gl.vertexAttribPointer(4, 1, gl.INT, false, 0, 0);
      gl.vertexAttribDivisor(4, 1);
    ...
    if (projPositionX) {
      ...
      gl.bindVertexArray(projVao);
      gl.bindBuffer(gl.ARRAY_BUFFER, projPosXVbo);
      gl.bufferData(gl.ARRAY_BUFFER, projPositionX, gl.DYNAMIC_DRAW);
      gl.bindBuffer(gl.ARRAY_BUFFER, projPosYVbo);
      gl.bufferData(gl.ARRAY_BUFFER, projPositionY, gl.DYNAMIC_DRAW);
      gl.bindBuffer(gl.ARRAY_BUFFER, projTypeVbo);
      gl.bufferData(gl.ARRAY_BUFFER, projType, gl.DYNAMIC_DRAW);
    ...
    if (itemInstanceX) {
      ...
      gl.bindVertexArray(itemVao);
      gl.bindBuffer(gl.ARRAY_BUFFER, itemPosXVbo);
      gl.bufferData(gl.ARRAY_BUFFER, itemInstanceX, gl.DYNAMIC_DRAW);
      gl.bindBuffer(gl.ARRAY_BUFFER, itemPosYVbo);
      gl.bufferData(gl.ARRAY_BUFFER, itemInstanceY, gl.DYNAMIC_DRAW);
      gl.bindBuffer(gl.ARRAY_BUFFER, itemOwnerTypeVbo);
      gl.bufferData(gl.ARRAY_BUFFER, itemInstanceOwnerType, gl.DYNAMIC_DRAW);
      gl.bindBuffer(gl.ARRAY_BUFFER, itemDefIdVbo);
      gl.bufferData(gl.ARRAY_BUFFER, itemInstanceDefId, gl.DYNAMIC_DRAW);
  ...
  function syncReact() {
    ...
    const inspectEntity =
      inspectEntityId === -1
        ...
        : {
            id: inspectEntityId,
            name:
              (window as any).entityNames?.[inspectEntityId] ||
              `Entity ${inspectEntityId}`,
    ...
    root.render(
      <App
        ...
        groupBuildingCount={groupBuildingCount}
        groupWood={groupWood}
        groupGold={groupGold}
        groupFood={groupFood}
        groupMisc={groupMisc}
        ...
        chronicle={chronicle}
        onFollow={() => {
          followEntityId = inspectEntityId;
        }}
        onClearInspect={() => {
          inspectEntityId = -1;
          followEntityId = -1;
  ...
  workers.forEach((w, i) => {
    w.onmessage = (e) => {
      ...
      if (type === "ENTITY_FOUND") {
        inspectEntityId = payload.id;
      }
      if (type === "MAGIC_BURST") {
        addChronicle(
          `Magic Burst from Group ${groupAffiliations[payload.entityId * 10] || "?"}`,
        );
      }
  ...
  import("./simulation/buffs").then((B) => {
    (window as any).Buffs = B;
  });
  ...
  async function handleSave() {
    const path = await save({
      filters: [{ name: "Sim", extensions: ["bin"] }],
    });
    if (!path) return;
