...
let positionX: Float32Array, positionY: Float32Array, health: Int32Array, money: Int32Array, state: Uint8Array, groupAffiliations: Int32Array, traitBitmask: Uint32Array;
...
let isLooping = false, isTickPending = false, lastTickStartTime = 0, totalTickTime = 0, tickCount = 0;
let cameraX = 0, cameraY = 0, zoomLevel = 1.0, isFollowing = false, selectedEntityId = -1;

interface MagicBurst { fromX: number; fromY: number; toX: number; toY: number; frames: number; }
let activeBursts: MagicBurst[] = [];

const root = createRoot(document.getElementById('react-root')!);
function renderUI() { root.render(<App ruleRegistry={ruleRegistry} groupPopulation={groupPopulationCount} groupTotalWealth={groupTotalWealth} tickCount={tickCount} />); }
...
worker.onmessage = (e: MessageEvent) => {
  ...
  if (d.type === "TICK_COMPLETE") {
    ...
    render(); if (isLooping) requestAnimationFrame(() => { if (isLooping && !isTickPending) { isTickPending = true; lastTickStartTime = performance.now(); worker.postMessage({ type: "TICK" }); } });
  }
...
canvas.addEventListener('click', (e) => {
    ...
    worker.postMessage({ type: "FIND_ENTITY", payload: { x: wx, y: wy, radius: 10 / zoomLevel } });
});

btnToggleLoop.addEventListener('click', () => { isLooping = !isLooping; btnToggleLoop.textContent = isLooping ? "Pause Loop" : "Start Loop"; if (isLooping && !isTickPending) { isTickPending = true; lastTickStartTime = performance.now(); worker.postMessage({ type: "TICK" }); } });
btnSingleStep.addEventListener('click', () => { if (!isLooping && !isTickPending) { isTickPending = true; lastTickStartTime = performance.now(); worker.postMessage({ type: "TICK" }); } });

saveBtn.addEventListener('click', async () => {
  worker.postMessage({ type: "PAUSE_SIM" });
  const buffers = [
    positionX, positionY, health, money, state, groupAffiliations, 
  ...
  if (path) await writeFile(path, buf); worker.postMessage({ type: "RESUME_SIM" });
});

loadBtn.addEventListener('click', async () => {
  const path = await open({ multiple: false }); if (!path || Array.isArray(path)) return;
  worker.postMessage({ type: "PAUSE_SIM" }); const data = await readFile(path);
  ...
  worker.postMessage({ type: "SYNC_TICK", tickCount: 0 }); worker.postMessage({ type: "RESUME_SIM" });
});

btnNationCommand.addEventListener('click', () => { worker.postMessage({ type: "GROUP_COMMAND", payload: { groupId: 5, commandState: 3, targetX: 800, targetY: 600 } }); });
btnArmyCommand.addEventListener('click', () => { worker.postMessage({ type: "GROUP_COMMAND", payload: { groupId: 25, commandState: 2, targetX: 100, targetY: 100 } }); });
